...
library(ggpubr)
library(dplyr)
setwd("G:/AI_XuLab_Data/native_chromatin_summary/Fig6/")

all_motif <- read.table(file = "K562_xchip_sites_motif.bed")
all_motif <- all_motif[nchar(all_motif$motif_chr) <= 5 & all_motif$XChIP >= 0.5 & all_motif$loMNase >= 0.5,]
all_motif$XChIP_ <- round(all_motif$XChIP,1)
all_motif$loMNase_ <- round(all_motif$loMNase,1)

# Group motifs based on XChIP and loMNase signals
...
for (index in all_motif_group_index){
  ...
  if (m_n >= 6) {
    ...
    if (all(unique(m_s1$site_type) == "native" && unique(m_s6$site_type) == "nonnative" && max(m_s4$salt_tolerance)<=2)){
      s1 <- rbind(s1, m_s1)
      s2 <- rbind(s2, m_s2)
      s3 <- rbind(s3, m_s3)
      s4 <- rbind(s4, m_s4)
...
write.table(s1[,c(1:19)], file = "sameXChIP_differNChIP_S1.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(s2[,c(1:19)], file = "sameXChIP_differNChIP_S2.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(s3[,c(1:19)], file = "sameXChIP_differNChIP_S3.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(s4[,c(1:19)], file = "sameXChIP_differNChIP_S4.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(s5[,c(1:19)], file = "sameXChIP_differNChIP_S5.txt",sep = "\t",col.names = T,row.names = F,quote = F)
write.table(s6[,c(1:19)], file = "sameXChIP_differNChIP_S6.txt",sep = "\t",col.names = T,row.names = F,quote = F)
...
create_boxplot <- function(data, y_var, y_lim, y_breaks, y_labels, output_pdf, output_png, y_title) {
  ...
  ggsave(output_pdf, plot = p, width = 4.3, height = 4.5, dpi = 600)  # 保存为pdf文件
  ggsave(output_png, plot = p, width = 4.3, height = 4.5, dpi = 600)  # 保存为png文件
}

create_boxplot(all_group, salt_tolerance, c(-0.5, 8.5), seq(0, 8, 2), seq(0, 8, 2),
               "boxplot_S1-S6_salt_tolerance_score_.pdf",
               "S1-S6_salt_tolerance_score_.png",
               "S-score")

create_boxplot(all_group, XChIP_, c(0, 28), seq(0, 30, 14), seq(0, 30, 14),
               "S1-S6_XChIP_signal.pdf",
               "S1-S6_XChIP_signal.png",
               "X-ChIP signal")

create_boxplot(all_group, loMNase_, c(0.5, 1.3), seq(0.5, 1.5, 0.4), seq(0.5, 1.5, 0.4),
               "S1-S6_loMNase_signal.pdf",
               "S1-S6_loMNase_signal.png",
               "loMNase-seq signal")
