...
# Sample 1
resolution = 10000
clr = cooler.Cooler('/mnt/disk3/husiling/private/data/native_chromatin_Micro-C/20230630_K562_Micro-C_JIACE_BJ_data_analysis/4hicpro/K562_MicroC_allvalidpairs/cooler_res10000/K562_MicroC_DMSO.cool')
...
# Sample 2
resolution = 10000
clr = cooler.Cooler('/mnt/disk3/husiling/private/data/native_chromatin_Micro-C/20230630_K562_Micro-C_JIACE_BJ_data_analysis/4hicpro/K562_MicroC_allvalidpairs/cooler_res10000/K562_MicroC_TPA.cool')
hg38_chromsizes = bioframe.fetch_chromsizes('hg38')
hg38_cens = bioframe.fetch_centromeres('hg38')
hg38_arms = bioframe.make_chromarms(hg38_chromsizes, hg38_cens)
hg38_arms = hg38_arms[hg38_arms.chrom.isin(clr.chromnames)].reset_index(drop=True)
...
axs[0].xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{x / 1e6:.1f}' if x % 1e6 != 0 else f'{int(x / 1e6):d}'))
axs[0].xaxis.set_tick_params(labelsize=14)

axs[1].xaxis.set_major_locator(plt.FixedLocator([1e5, 1e6, 1e7, 1e8]))
axs[1].xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f'{x / 1e6:.1f}' if x % 1e6 != 0 else f'{int(x / 1e6):d}'))
...
# Save the figure
plt.savefig("contacts_vs_distance/cooler_IC_contact_frequency_DMSO_TPA.pdf", bbox_inches='tight')
plt.savefig("contacts_vs_distance/cooler_IC_contact_frequency_DMSO_TPA.png", dpi=600, bbox_inches='tight')
