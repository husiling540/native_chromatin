...
for prefix in ${prefixes}; do
  ...
  LANG=en
  awk -F "\t|;" '{$10=$2","$3;print $1";"$2","$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10"\t"$11"\t"$12"\t"$13"\t"$14}' ${workdir}/4hicpro/${prefix}_hicpro/hic_results/data/${prefix}/${prefix}_hg38XX.bwt2pairs.validPairs | sort -T /mnt/disk3/husiling/private/data/tmp/ -S 50% -k2,2V -k3,3n -k5,5V -k6,6n -k8,8n -m | awk -F "\t|;" 'BEGIN{c1=0;c2=0;s1=0;s2=0;a1=0}(c1!=$3 || c2!=$6 || s1!=$4 || s2!=$7 || a1!= $2){print;c1=$3;c2=$6;s1=$4;s2=$7;a1=$2}' >${workdir}/4hicpro/${prefix}_hicpro/hic_results/data/${prefix}/${prefix}.UMI.allValidPairs
